...
export const FormActivity = () => {
  ...
  const getActivity = async (id) => {
    ...
    const data = await activityService.get(id)
    console.log(data)
    if (data) {
  ...
  const onFinish = async (values) => {
    ...
    if (showEdit)
      ...
      await activityService.insert(data)
  ...
  const fetchMovements = async (page, pageSize) => {
    ...
    setLoading(true);
    try {
      const response = await activityService.list(page, pageSize);
        ...
        <Row >
          <Col span={10} className="p-2">
            <Card style={{ overflow: 'auto', height: '500px' }} >
              <div className="text-center"><h3>Datos de la actividad</h3></div>
              <Form
                name="productForm"
                labelCol={{ span: 8 }}
                wrapperCol={{ span: 24 }}
                ...
                <Spin spinning={loading}>
                  <div style={{ display: "flex", justifyContent: "center" }}>
                    <Button type="primary" htmlType="submit" hidden={showEdit}  >
                      <i className="bi bi-save fa-lg m-2" /> Guardar actividad
                    </Button>
