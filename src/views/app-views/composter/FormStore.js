...
export const FormStore = () => {
  ...
  const onFinish = async (values) => {
    ...
    if (showEdit)
      ...
      await storeService.insert(data)
  ...
  const fetchMovements = async (page, pageSize) => {
    ...
    setLoading(true);
    try {
      const response = await storeService.list(page, pageSize);
  ...
  return (
    <>
      <div className=" row ">
        <Divider orientation="left">Bodegas</Divider>
        <Row >
          <Col span={10} className="p-2">
            <Card style={{ overflow: 'auto', height: '500px' }} >
              <div className="text-center"><h3>Datos de la bodega</h3></div>
              <Form
                name="productForm"
                labelCol={{ span: 8 }}
                wrapperCol={{ span: 24 }}
              ...
              >
                ...
                <Form.Item label="Nombre" name="name" rules={rules.name}>
                  <Input name="name" placeholder="Nombre de la bodega" />
                </Form.Item>
                ...
                <Spin spinning={loading}>
                  <div style={{ display: "flex", justifyContent: "center" }}>
                    <Button type="primary" htmlType="submit" hidden={showEdit}  >
                      <i className="bi bi-save fa-lg m-2" /> Guardar bodega
                    </Button>
