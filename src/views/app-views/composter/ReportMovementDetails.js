...
export const ReportMovementDetails = () => {
  ...
  const downloadExcel = async () => {
    try {
        const response = await reportService.generateExcelMovementDetails(startDate,endDate)
  ...
  const columns = [
    ...
    {
      title: "Maquina",
      dataIndex: "movement_details",
      key: "description",
      render: (details) => details?.map(detail => detail.machineName).join(", ") || "-"
    },
    {
      title: "Tiempo/min",
      dataIndex: "movement_details",
      key: "description",
      render: (details) => details?.map(detail => detail.time).join(", ") || "-"
    },
    {
      title: "Descripción",
      dataIndex: "movement_details",
      key: "description",
      render: (details) => details?.map(detail => detail.description).join(", ") || "-"
    }
  ];
  ...
  const fetchMovements = async (page, pageSize) => {
    ...
    setLoading(true);
    try {
      const response = await reportService.getReportMovementDetails(page, pageSize, start, end)
      if (response.success) {
  ...
  return (
    <>
      <div className=" row ">
        <Divider orientation="left">Reporte de actividades</Divider>
        <Row >
...
export default ReportMovementDetails;
